...
export const LocationScreen: React.FC<Props> = ({ focusMatchId, onBack }) => {
  ...
  const fetchCloudMatches = async () => {
    ...
    try {
      ...
      setCloudMatches(await fetchAllCloudMatches(db));
    } catch (error) {
  ...
  const centerOnMatch = (id: string | null) => {
    ...
    const match = activeHistory.find((item) => item.id === id);
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      activeHistory
        .filter((match) => match.location && typeof match.location.lat === 'number' && typeof match.location.lng === 'number')
        .forEach((match) => {
      ...
      setTimeout(() => {
        ...
        const markerList = Object.values(markersRef.current) as L.Layer[];
