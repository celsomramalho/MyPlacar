...
export const HistorySection: React.FC<Props> = ({ 
  ...
  const filtered = history.filter(item => 
    !searchQuery || 
    item.p1Name.toLowerCase().includes(searchQuery.toLowerCase()) || 
    item.p2Name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    item.winner.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const realUnsyncedCount = useMemo(() => history.filter(h => !h.isSynced).length, [history]);
  const syncedLocalCount = useMemo(() => history.filter(h => h.isSynced).length, [history]);
  const totalCloudCount = syncedLocalCount + cloudMatchesCount;

  const grouped = useMemo(() => {
    const groups: Record<string, MatchHistoryItem[]> = {};
  ...
  return (
    <div className="flex flex-col gap-4 animate-in fade-in duration-300 pb-32 font-sans">
           ...
           <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button 
                  ...
                  className={`w-14 h-14 rounded-2xl transition-all shadow-lg flex items-center justify-center border-2 ${
                    (isSyncingAll || history.length === 0)
                    ? 'bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed opacity-50' 
                    : 'bg-amber-500 text-white border-amber-400 active:scale-90 shadow-amber-100'
                  }`}
                >
                ...
                <button 
                  ...
                  className={`w-14 h-14 rounded-2xl transition-all shadow-lg flex items-center justify-center border-2 ${
                    (isDownloading || cloudMatchesCount === 0)
                    ? 'bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed opacity-50' 
                    : 'bg-emerald-600 text-white border-emerald-500 active:scale-90 shadow-emerald-100'
                  }`}
                >
      ...
      {filtered.length === 0 ? (
        ...
        Object.entries(grouped).map(([originalDate, val]) => {
          const items = val as MatchHistoryItem[];
          return (
            <div key={originalDate} className="space-y-4">
              ...
              {items.map(item => {
                ...
                const p1SetsWon = setsP1.filter((s, i) => s > setsP2[i]).length;
                const p2SetsWon = setsP2.filter((s, i) => s > setsP1[i]).length;
                        ...
                        <div className="flex items-center gap-3">
                           ...
                           <div className="flex items-center gap-2 px-2.5 py-1.5 bg-gray-50 rounded-xl border border-gray-100">
                             <LazySportIcon 
                               ...
                               defaultIcon={item.sportType === 'tennis' ? '🎾' : '🥒'} 
