...
export const ScoreboardScreen: React.FC<Props> = (props) => {
  ...
  const { isListening, start, stop } = useGeminiReferee({
    ...
    onSwitchServer: () => onSwitchServer(gameState.server === 1 ? 2 : 1, false), onAnnounceScore: announceFullScore, isEnabled: gameState.matchConfig.voiceEnabled && !voiceWasManuallyStopped && !gameState.isLiveClosed && isCommandOwner && !gameState.isMatchOver && !gameState.matchConfig.isWatchMode && !isWatchDevice(),
